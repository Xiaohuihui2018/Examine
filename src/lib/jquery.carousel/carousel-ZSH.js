        ...
        //初始化
           function init(){
              ...
              var $html=$.map($opts.imgs,function(url){
                return '<li><img src="'+url+'" width="'+$opts.width+'" height="'+$opts.height+'"/></li>'
              }).join('');
                    ...
                    if($opts.marquee){
                        ...
                        if($opts.page==true){
                            if($opts.index==$imgs_len){
                                $this.find('span').eq(0).addClass('active').siblings().removeClass('active');
                            }              
                            $this.find('span').eq($opts.index).addClass('active').siblings().removeClass('active'); 
                        }    
                    }else{
                    //有缝点击及淡化切换
                        console.log('有缝');
                ...
                switch($opts.type){
                    ...
                    if($opts.marquee==true){
                        ...
                        timer=setInterval(function(){
                           ...
                           $this.children('ul').stop(true,true).animate({left:-$opts.index*$opts.width},700);
                           //页码 
                           if($opts.page==true){
                                if($opts.index==$imgs_len){
                                 $this.find('span').eq(0).addClass('active').siblings().removeClass('active');
